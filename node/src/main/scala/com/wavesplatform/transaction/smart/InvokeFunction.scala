  ...
  override def buildExpression(state: State, args: List[EVALUATED]): Either[ExecutionError, EXPR] = state match {
    case ds: DAppState =>
      val invocationExpression: Either[ExecutionError, (EXPR, (InvokeScript, PublicKey, StdLibVersion))] = args match {
        case dappAddressOrAlias :: function :: ARR(invokeArgs) :: ARR(payments) :: Nil =>
          val dappAddress = dappAddressOrAlias match {
            case CaseObj(`addressType`, fields) =>
              fields
                .get("bytes")
                .collect { case bs: CONST_BYTESTR => bs.bs.arr }
                .fold(CommonError(s"object $dappAddressOrAlias has no 'bytes' field").asLeft[Address]) { bs =>
                  Address.fromBytes(bs).leftMap(ia => CommonError(ia.reason))
                }
            case CaseObj(`aliasType`, fields) =>
              fields
                .get("alias")
                .collect { case al: CONST_STRING => al.s }
                .fold(CommonError(s"object $dappAddressOrAlias contains no 'alias' field").asLeft[Address]) { aliasName =>
                  Alias
                    .create(aliasName)
